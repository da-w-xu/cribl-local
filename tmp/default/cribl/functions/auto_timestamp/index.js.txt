...
exports.process = (e) => {
  ...
  const earliest = C.Time.parseRelativeTime(conf.earliestDateAllowed || C.Time.EARLIEST_DATE_RANGE);
  const latest = C.Time.parseRelativeTime(conf.latestDateAllowed || C.Time.LATEST_DATE_RANGE);
